...
def get_BPAG():
    ...
    for index in range(len(bpag_data)):
        ...
        elif(bpag_data[index] =='Moderate'):
            new_bpag_data.append(1)
        elif(bpag_data[index] =='Severe'):
...
def get_raw_BPAG():
    ...
    bpaq_physical_aggression = data['OFER_AcuteFatigue_Score'].tolist()
 
    return bpaq_physical_aggression
...
if __name__ == '__main__':
   ...
   #  model.compile(optimizer='adam', loss='mean_squared_error', metrics=['mae'])
    ...
    # 모델 훈련
    history = model.fit(X_train, y_train, epochs=50,validation_split= 0.2, batch_size=32)  # 에포크 수 및 배치 크기는 상황에 맞게 조정하세요.
